...
网关上需要安装 AgentIdentity WASM 插件并配置规则。若策略集是通过**控制台**绑定的，可能会默认安装一个 AgentIdentity 鉴权插件，需先卸载再安装当前版本；若通过 **CLI/API** 绑定策略集，则网关插件列表为空（无默认插件），无需卸载，直接安装即可。

### 6.1 卸载默认 AgentIdentity 插件（仅控制台绑定路径需要）

1. 进入网关实例 → **插件市场** → **已安装插件**
2. 若存在策略集自动绑定的 **AgentIdentity** 插件，点击 **卸载**（CLI/API 绑定路径下此处为空，跳过即可）
3. 安装当前的 AgentIdentity WASM 插件（插件正式发布后可直接从插件市场安装；过渡期内需手动上传构建产物，见 README 的"WASM 插件获取方式"）

> **说明**：插件正式版本发布后，不需要再手动上传插件，可以直接使用官方提供的插件。

![插件配置页面](images/12-plugin-config-page.png)

### 6.2 配置 AgentIdentity 插件规则

插件安装后，需要配置规则使其在指定的 MCP 服务上生效。

1. 进入网关实例 → **插件** → 找到已安装的 AgentIdentity 插件 → 点击 **规则配置**
