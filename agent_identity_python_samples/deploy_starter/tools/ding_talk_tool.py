...
async def on_auth(url: str):
    await AgentContext.on_auth_url(url, "Write to DingTalk document")
    ...
    try:
        ...
        return ToolResponse(
            content=[
                TextBlock(
                    type="text",
                    text=f"Successfully wrote to DingTalk document, response: {result}",
                ),
            ],
        )
        ...
        return ToolResponse(
            content=[
                TextBlock(
                    type="text",
                    text=f"Failed to write to DingTalk document: {str(e)}",
                ),
            ],
        )